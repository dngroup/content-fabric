...
try:
    ...
    gateway=[item["IPAM"]["Config"][0]["Gateway"] for item in cli.networks() if item["Name"]==DOCKER_COMPOSE_NETWORK_NAME][0]
    ...
    res=pool.map(experiment,zip(10000+rs.randint(0,PEER_COUNT,CLIENT_COUNT)*10,(np.cumsum(rs.poisson(ARRIVAL_TIME,CLIENT_COUNT)))))
